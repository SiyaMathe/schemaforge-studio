...
AS
    SELECT
        ...
        FROM dbo.OrderLine ol
        WHERE ol.OrderID = o.OrderID) AS TotalItems,
...
AS
    ...
    END AS DeliverySuccessRate,
        ...
        (SELECT COUNT(*)
        FROM dbo.Address a
        WHERE a.CustomerID = c.CustomerID) AS AddressCount,
...
AS
    ...
    SELECT
        ...
        (SELECT COUNT(*)
        FROM dbo.Product p
        WHERE p.VendorID = v.VendorID AND p.IsActive = 1) AS ActiveProducts,
...
CROSS APPLY (
    SELECT
            ...
            -- FIX: Outer reference factor pulled outside of SUM expression block
...
PRINT 'Operational views created successfully.';
GO